...
/***  Exercises 9.4 - Problem 2: y' = 4x - 2y, y(0) = 2, y(0.5)  ***/
...
double k2(double xn, double yn, double h, double k1) {
	return h*(4 * (xn + 0.5*h) - 2 * (yn + 0.5*k1));
}

double k3(double xn, double yn, double h, double k2) {
	return h*(4 * (xn + 0.5*h) - 2 * (yn + 0.5*k2));
}

double k4(double xn, double yn, double h, double k3) {
	return h*(4 * (xn + h) - 2 * (yn + k3));
}

int main()
{
	ofstream outfile("RK4results.txt", std::ofstream::out);

	double xn = 0, yn = 2, h = 0.1; //initial values
	outfile << setprecision(5) << yn << ", " << xn << endl; //set precision to 4 decimal places
	...
	xn += h;
	outfile << setprecision(5) << ynext << ", " << xn << endl; //set precision to 4 decimal places
